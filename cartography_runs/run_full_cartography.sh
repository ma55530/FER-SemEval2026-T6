...
CONTAINER_NAME="${CONTAINER_NAME:-cartography-full}"
...
if [ "$MODE" = "docker" ]; then
  ...
  python scripts/semeval_cartography.py "${EXTRA_ARGS[@]}"
else
  echo "Usage: bash run_full_cartography.sh [docker|native] [script args...]"
  exit 1
fi
